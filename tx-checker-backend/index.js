...
const BLOCKBOOK_API =
  process.env.BLOCKBOOK_API || "https://eth-blockbook.nownodes.io";
const MODEL_API =
  process.env.MODEL_API ||
  "https://functions.yandexcloud.net/d4efncogm7e0s3icpa2e";
const API_KEY = process.env.API_KEY || "";

app.post("/transaction", async (req, res) => {
  try {
    const { txId, direction } = req.body;
    if (!txId || !direction) {
      return res.status(400).send("Transaction ID and direction are required.");
    }

    console.log(`Incoming ${direction} transaction`, txId);
